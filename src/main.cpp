...
float co = 0.0, pm1_0 = 0.0, pm2_5 = 0.0, pm10 = 0.0;
float temperature = 20.0, humidity = 65.0;
float lat = 0.0;
float longti = 0.0;
...
void loop()
{
  ...
  if (disp.get_state_function_icon(FUNCTION_ICON::CALL) == true && millis() - previous_millis_call >= 7000)
  {
    ...
    sim7680c.callnumber(String(PHONE_NUMBER_DEFAULT));
  ...
  if (millis() - previous_millis_call >= TIME_CHUP_CALL && disp.get_state_function_icon(FUNCTION_ICON::CALL) == true)
  {
    ...
    disp.display_notification("Called");
  ...
  if (disp.get_state_function_icon(FUNCTION_ICON::SMS) == true)
  {
    ...
    sim7680c.sendSMS(String(PHONE_NUMBER_DEFAULT), "SMS WARNING");
