...
void display_tft_espi::background()
{
    ...
    for (int i = 0; i < sizeof(title_display_tft) / sizeof(title_display_tft[0]); i++)
    {
        ...
        draw_button(FIRST_LINE_X + 200, FIRST_LINE_Y + i * LINE_WIDTH, BUTTON_COMPONENT_WIDTH, BUTTON_COMPONENT_HEIGHT, TFT_GREEN);
        draw_wifi_icon(FIRST_ICON_X, FIRST_ICON_Y, ICON_RADIUS, false);
        draw_4g_icon(FIRST_ICON_X, FIRST_ICON_Y + 50, ICON_RADIUS, false);
        draw_sms_icon(FIRST_ICON_X, FIRST_ICON_Y + 100, ICON_RADIUS, false);
        draw_call_icon(FIRST_ICON_X, FIRST_ICON_Y + 150, ICON_RADIUS, false);
    }
...
void display_tft_espi::process_touch()
{
    ...
    if (tft.getTouch(&x, &y) && (currentTouchTime - lastTouchTime > TIME_TOUCH_DEBOUNCE)) // Kiểm tra chạm và thời gian debounce (200ms)
    {
        ...
        for (int i = 0; i < 9; i++)
        {
            ...
            uint16_t buttonX = FIRST_LINE_X + 200; // Đặt nút bên phải của tiêu đề
...
void display_tft_espi::PM(float PM2_5, float PM10)
{
    // Display PM2.5
...
void display_tft_espi::GPS(float latitude, float longitude, char *time)
{
    // Display Latitude
