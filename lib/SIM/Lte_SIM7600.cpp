...
bool Sim7600G::callnumber(String number)
{
  number += ";";
  sendAT("D", number);
  return waitResponse(1000L);
}

bool Sim7600G::sendSMS(String number, String message)
{
  sendAT("+CMGF=?");
  if (waitResponse(1000L, "+CMGF:") != 1)
  {
    sendAT("+CMGF=1");
...
// HARDWARE RELATED
int Sim7600G::readTemperatureModule()
{ ////////verificar es float
  sendAT("+CPMUTEMP");
...
bool MqttClient::connected()
{
  ...
  if (waitResponse(1000L, "+CMQTTDISC: 1,1") != 1)
  {
  } // catch +CMQTTDISC: 1,1
...
bool MqttClient::connect(String cient_id, String user, String pass, String server, int32_t port, int8_t tcp_ssl, int8_t contexSSL, int8_t sessionId, int8_t sslCtxIndex)
{
  ...
  sendAT("+CMQTTCONNECT=0,\"", broker, "\"", ",60,1,", "\"", user, "\",\"", pass, "\"");
  if (waitResponse(6000L, "+CMQTTCONNECT:") != 1)
  {
    response = 1;
  }
  String res = LTE.readStringUntil('\n');
...
int8_t MqttClient::subscribe(String topic, int16_t topic_length, int8_t qos)
{
  ...
  if (waitResponse(1000L, "+CMQTTSUB:") != 1)
  {
  } // catch Error
  streamSkipUntil(','); /* Skip format (0) */
...
int8_t MqttClient::publish(String topic, String payload, int16_t topic_length, int16_t payload_length, int8_t qos)
{
  ...
  sendAT("+CMQTTPUB=0,", String(qos), ",60,0");
  if (waitResponse(1000L, "+CMQTTPUB:") != 1)
  {
  } // catch Error
  streamSkipUntil(','); /* Skip format (0) */
  int8_t error = streamGetIntBefore('\n');
...
int16_t HttpsClient::post(String data)
{
  ...
  if (waitResponse(6000L, "+HTTPACTION:") != 1)
  {
    return 0;
  }
  String res = LTE.readStringUntil('\n');
...
String HttpsClient::getString()
{
  ...
  if (waitResponse(1000L, "+HTTPREAD:") != 1)
  {
    response = "";
  }
  if (waitResponse(1000L, "DATA,") != 1)
  {
    response = "";
  }
  response = LTE.readStringUntil('+');
...
{
  sendAT("+CCERTLIST");
  if (waitResponse(1000L, "+CCERTLIST:") != 1)
  {
    return false;
  }
